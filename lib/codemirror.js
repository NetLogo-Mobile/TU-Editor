    ...
    on(d.scroller, "touchend", function (e) {
      ...
      if (touch && !eventInWidget(d, e) && touch.left != null &&
        ...
        if (!touch.prev || farAway(touch, touch.prev)) // Single tap
          ...
          { range = cm.findWordAt(pos); origin="double-tap" }
        else // Triple tap
          { range = new Range(Pos(pos.line, 0), clipPos(cm.doc, Pos(pos.line + 1, 0))); origin="triple-tap" }
        cm.setSelection(range.anchor, range.head, { origin: origin });
        cm.focus();
        e_preventDefault(e);
      }
      finishTouch();
